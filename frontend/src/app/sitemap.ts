...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [prodData, catData, blogData, brandData] = await Promise.all([
  ...
  const brands = brandData.data || brandData.brands || []

  const staticRoutes: MetadataRoute.Sitemap = [
    { url: BASE_URL,                         lastModified: new Date(), changeFrequency: 'daily',   priority: 1.0 },
    { url: `${BASE_URL}/products`,           lastModified: new Date(), changeFrequency: 'daily',   priority: 0.9 },
    { url: `${BASE_URL}/offers`,             lastModified: new Date(), changeFrequency: 'daily',   priority: 0.8 },
    { url: `${BASE_URL}/dosha-quiz`,         lastModified: new Date(), changeFrequency: 'monthly', priority: 0.8 },
